...
- **FR-DE-7 (Catchment delineation)** — For each reservoir, obtain the **upstream contributing catchment** by selecting the **HydroSHEDS HydroBASINS** polygon(s) containing the dam (GEE `WWF/HydroSHEDS/v1/Basins/hybas_*`), with MERIT Hydro / `pyflwdir` as a finer fallback; persist the catchment polygon (PostGIS). Done once per reservoir, re-runnable on config change, with manual-override support.
- **FR-DE-8 (Precipitation ingestion & aggregation)** — Ingest open precipitation (CHIRPS / GPM IMERG / ERA5-Land) via `geemap`/`xee` and compute **catchment-aggregated precipitation** per time step, plus engineered features: lagged precip and an **antecedent precipitation index** capturing routing delay between rainfall and reservoir response.
...
- **FR-GT-4 (Empirical rating-curve construction)** — Use the matched pairs to **build the reservoir's area↔storage↔level relationship directly from observed data**, regressing **extracted surface area** against the historical fields — **current reservoir level (m)**, **current live storage (BCM)**, and **`pct_filled`** — and anchoring the fit with **FRL** and **full live capacity at FRL** (area/storage/level at FRL define the curve's top bound). This yields per-reservoir **Area→Storage (BCM)** and **Area→Level (m)** functions that inherently absorb SAR-extraction bias and real basin behaviour. The DEM-derived hypsometric curve serves as a **physics prior and extrapolation backstop** for fill states the history does not cover (notably the near-FRL monsoon-peak / release-risk range). Persist the calibrated curve, versioned, with its valid (observed) range flagged.
- **FR-GT-5 (Extraction-method validation & selection)** — Compare candidate extraction methods (e.g. K-means vs Otsu vs segmentation model) on the matched set, and **select/promote** the method whose derived volume/level best agrees with ground truth. Track all candidates and metrics in MLflow.
...
- **FR-ML-3 (Release prediction — primary)** — Derive a **probability/likelihood of water release** per reservoir over the forecast horizon, computed from the forecast fill-level trajectory relative to FRL and reservoir-specific release thresholds (with prediction uncertainty). Output a discrete **release-risk level** (e.g. Low / Watch / Warning / Imminent) and the **estimated lead time** to a likely release.
...
- **FR-ML-6** — Continuously **validate** live predictions (including release-risk calls) against incoming ground truth / observed releases, and surface accuracy and drift metrics.
...
- **NFR-ACC-3 (release prediction — primary)** — Release-risk prediction shall be optimised for **high recall on actual flood/emergency release events** (minimise missed releases — false negatives are the costliest outcome) while keeping false-alarm rate acceptable, and shall provide a **minimum useful lead time** (target: flag likely releases **≥ 2–3 days ahead**, final threshold set during validation against observed releases). Metrics use **precision-recall** (not ROC) given event rarity, and release probabilities shall be **well-calibrated**.
- **NFR-ACC-4** — Accuracy metrics (estimation, forecast skill, release recall/precision, lead time, calibration) are computed continuously and visible in the dashboard.
...
- **DEM prior / backstop:** the DEM-derived hypsometric curve provides a physics prior and extrapolates beyond the observed fill range (near-FRL release-risk states the history may not cover).
...
- **Approach:** time-series forecasting per reservoir with **prediction intervals**; benchmarked against persistence **and climatology** baselines. Model family to be selected during experimentation (e.g. classical time-series vs gradient-boosted/ML regressors vs sequence models) and chosen by validation skill.
...
- Continuous post-hoc validation of live predictions against incoming bulletin ground truth.
...
The available history is small — roughly weekly bulletins, three reservoirs, predominantly a single low-storage season — and release events are rarer still. This regime governs every modelling choice:
...
- **AC-5 (primary)** — Release-risk prediction meets NFR-ACC-3: on the held-out set of historical near-FRL/release episodes it achieves the agreed recall and minimum lead time, with calibrated probabilities and a clear risk level per reservoir.
...
- **AC-11** — Release-risk alerts are **delivered through external channels** (email + at least one of SMS/webhook) with logged delivery status, role-based subscriptions, and rate-limiting.
- **AC-12** — Automated data-validation and the regression backtest (§7.8) pass in CI.

**Operational KPIs:** **release-event recall, false-alarm rate, and mean warning lead time (headline)**; pipeline success rate; data freshness lag; estimation MAE; forecast skill vs baseline; API p95 latency; service uptime.
...
- The historical records provide FRL, current level, full live capacity, and current live storage (BCM) per date — sufficient to build the empirical area↔storage↔level relationship — though they may not span the full fill range (esp. near-FRL).
- A seasonal operating rule curve (or a reasonable proxy) is obtainable per reservoir to separate routine operational releases from flood/emergency spills.
...
| Empirical curve extrapolates beyond observed fill range (esp. near-FRL release zone) | Flag extrapolated estimates; fall back to DEM hypsometric prior; widen uncertainty; prioritise collecting near-FRL matches as new monsoon data arrives |
