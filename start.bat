...
echo [5/7] Populating data with the full pipeline (one-time, ~30s)...
call uv run python scripts/bootstrap.py
if errorlevel 1 ( echo   ERROR: bootstrap failed. & pause & exit /b 1 )

echo [6/7] Fetching REAL satellite overlays from Google Earth Engine (~1-2 min)...
...
echo [7/7] Starting API (port 18000) and web dashboard (port 5173)...
