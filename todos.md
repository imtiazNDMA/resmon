...
- [~] Define typed Pydantic schemas and TypeScript contracts for PMD station observations, NWFC observations, warning polygons, monsoon warnings, lightning strikes, GLOF observations, city forecasts, and FFD water levels — backend Pydantic contracts added for all 8C.3 endpoints; TypeScript still open
- [x] Represent nested JSON-string upstream fields (`fc`, `elements`, `gauges`, selected warning geometry fields) as parsed native JSON in our API responses where safe and stable — FFD `gauges`, monsoon/city `fc`, and warning geometry parsed
- [~] Preserve raw upstream IDs/codes (`station_id`, `code`, `data_type`, `model`, `forecast_time`) so map features can be joined, inspected, and refreshed deterministically — backend endpoints preserve observed IDs/codes/model/timing fields where present
- [x] Add freshness fields to every PMD response: `source`, `source_timestamp`, `fetched_at`, `cache_status`, `stale`, and `ttl_seconds` — all 8C.3 GeoJSON endpoints include freshness fields
- [x] Return empty valid FeatureCollections for normal no-data conditions; reserve non-2xx responses for auth/config/server failures — covered for all 8C.3 endpoints
- [~] Add fixture responses for representative non-empty, empty, stale-cache, sentinel-value, and malformed-nested-JSON cases — no-network backend tests cover non-empty, empty, stale, sentinel cleaning, and nested JSON parsing for current endpoints
...
- [x] Add `/weather/pmd/nwfc/observations` for NWFC station observations and weather-text/icon classification inputs
- [x] Add `/weather/pmd/monitor/warnings` for active warning polygons with severity, hazard element, model, forecast time, data time, message, and area
- [x] Add `/weather/pmd/monitor/monsoon` for monsoon-specific warnings and rainfall forecast text
- [x] Add `/weather/pmd/monitor/glof-observations` for GLOF station readings, connectivity, alert level, rainfall, flow, water level, and temperature
- [x] Add `/weather/pmd/monitor/lightning?hours=1..48` for recent lightning strike points
- [x] Add `/weather/ffd/waterlevels` for FFD river gauge discharge/status points and parsed gauge arrays
- [x] Add `/weather/pmd/monitor/city-forecast` for city current conditions plus parsed 12-step forecast arrays
- [x] Add endpoint-level timeout handling so one slow upstream source cannot block unrelated reservoir/hydrologic map endpoints — PMD client uses per-request timeout and route-local 502/503 handling
- [x] Add repository-free API tests using mocked PMD client responses; avoid network calls in tests — all 8C.3 endpoints covered with dependency override fake client

### 8C.4 PMD Forecast Raster Discovery

- [~] Confirm which PMD Monitor prediction element codes are live and stable before implementing each raster layer; do not infer codes from display labels alone — discovery contract, candidate registry, and `scripts/discover_pmd_prediction_elements.py` added
- [~] Confirm the four WRFPRS precipitation accumulation elements: `HOURTPE`, `SIXTPE`, `TWELVETPE`, `DAYTPE` — listed as candidates only; script can query `/api/modelTimeList` when credentials are configured
- [~] Discover and verify codes for 7d precipitation, cumulative precipitation, 6h/12h/24h rainfall-and-snow, and temperature/precipitation anomaly layers before adding them to the product UI — explicitly deferred until live portal metadata/raster evidence is available; not added to UI
- [~] Download one real GeoTIFF per confirmed element and inspect band count, CRS, nodata value, pixel value range, extent, and unit using GDAL — offline inspection script added; no live raster inspected yet
- [~] Extract official PMD color ramps from the source portal JavaScript where available; clearly label any fallback color ramp as our own choice, not PMD-official — registry uses `project_precip_fallback`; no PMD-official ramp claimed
- [~] Record confirmed elements, units, accumulation windows, color stops, source run cadence, and known caveats in a tracked contract document — `docs/contracts/pmd-forecast-raster-discovery.md` started with operator commands; candidate registry mirrors the contract; no elements confirmed yet

### 8C.5 Forecast Raster Processing + Tile/Image Serving

- [x] Add a forecast-raster element registry keyed by stable frontend IDs (`pmd_pred_hourtpe`, etc.) with upstream `data_type`, `element`, label, unit, accumulation window, color ramp, and max frame policy
- [~] Add backend endpoint `/weather/pmd/predictions/{element_key}` returning `{element, label, unit, run, steps}` where each step has `date`, `bounds`, `coordinates`, `url`, and `cache_status` — metadata scaffold added; `url` remains null until raster conversion is validated
- [x] Fetch latest model runs using `/api/modelTimeList?data_type=<data_type>&element=<element>` with 30-minute metadata TTL
- [x] Fetch per-run frame lists using `/api/model?data_type=<data_type>&element=<element>&date_time=<run>` with 3-hour metadata TTL
- [x] Thin forecast frames for browser performance: dense early window, sparse later window, with element-specific caps to avoid hundreds of simultaneous image overlays
- [~] Convert raw GeoTIFF frames server-side with GDAL: download bytes, write temp GeoTIFF, warp to EPSG:3857, colorize with fixed ramp, add alpha, persist PNG + JSON sidecar — offline raster inspection exists; conversion blocked until live raster artifact and official/fallback ramp decision are validated
- [~] Compute image bounds from all four warped raster corners; do not use a two-corner shortcut — required in contract; blocked until warp/conversion implementation
- [~] Use deterministic on-disk cache keys per `(element, run, forecast_time)` so repeated requests avoid network/GDAL work — cache policy documented; PNG/JSON conversion cache not implemented until conversion is validated
- [~] Delete intermediate GeoTIFF files after conversion; retain only PNG, metadata sidecar, and optional ramp files — required in contract; blocked until conversion implementation
- [x] Add disk-cache cleanup policy or operator command for old PMD prediction PNG/JSON files — `scripts/cleanup_pmd_prediction_cache.py`
- [x] Add optional `?diag=1` or admin-only cache bypass for operational debugging without exposing raw upstream probes publicly — deliberately not exposed in public API; operator scripts provide diagnostics without raw browser probes

### 8C.6 Weather Layer Persistence + Catchment Aggregation

- [x] Decide which PMD datasets remain cache-only display layers and which are persisted for modelling; avoid persisting noisy point observations unless they feed a defined product — `docs/contracts/pmd-weather-persistence-and-aggregation.md`
- [x] Add optional `weather_layer_provenance` or reuse `hydrologic_layer_provenance` only if semantics remain clear; do not conflate static hydrologic provenance with dynamic weather freshness — decision: use dedicated future weather freshness metadata, do not reuse hydrologic provenance
- [~] Add catchment aggregation job for PMD forecast rasters: clip/average/sum precipitation over `reservoir.catchment_geom` and optionally sub-basins where resolution supports it — blocked until raster elements are product-ready and units/CRS are validated
- [x] Write PMD-derived catchment precipitation forecasts to `forecast_forcing` only after unit/time-window alignment is explicit and tested — gate documented; no writes implemented
- [x] Write observed/reanalysis-like PMD rainfall summaries to `catchment_forcing` only when source cadence and spatial representativeness are documented — gate documented; no writes implemented
- [x] Keep PMD visual layers separate from validated forecasting inputs until walk-forward skill/backtest impact is measured
- [~] Add source-version/freshness flags to forcing rows so downstream ML can distinguish ERA5/Open-Meteo/GFS/PMD-derived inputs — required by contract for any future PMD-derived forcing rows; no schema change until forcing promotion is approved

### 8C.7 Frontend Map Layer UX

- [~] Add a `Weather` group in map controls separate from `Hydrologic layers`, with toggles for observations, warnings, lightning, GLOF, FFD gauges, and PMD prediction rasters — Weather group started with PMD station toggle
- [~] Add weather layer chips that are disabled with clear copy when backend credentials/config are missing, instead of failing silently — PMD station chip handles backend 503 credential state
